...
class ReportExporter
  ...
  def put_values
    get_values
    File.open("report_#{DateTime.now}.html", 'w') do |report|
      ...
      report.puts "Среднее число комментариев: #{Comment.all.length / Article.all.length} <br>"
      report.puts "<a href='http://localhost:3000#{article_path(@article.id)}'>#{@article.title}</a><br>"
...
task :user_list => :environment do
  ...
  @authors = Author.all
  CSV.open("user_list_#{DateTime.now}.csv", 'w') do |file|
    file << attributes
